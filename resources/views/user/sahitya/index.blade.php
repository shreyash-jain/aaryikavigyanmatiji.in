...
@section('content')
		<div class="main-body">
			<section class="wakil-blog-layout section-p-100">
				...
				@endphp
					<h2 class="color-text color-blue mb-25 ds">{{$category->name}}</h2>
					<div class="row">
					...
					@endphp
								...
								@else 
									<a href="{{$val->file}}" target="_blank" title="{{$val->title}}" class="count_view"  data-link="{{$val->file}}" data-bs-toggle="popover" data-bs-title="{{$val->title}}" data-bs-trigger="hover" >
								@endif	
										<div class="blog__box">
											<div class="blog_image position-relative">
												...
												<div class="overlay-bottom position-absolute  text-black text-center" style=" bottom: 0; right: 0; padding-bottom: 10px; padding-right: 10px;">
													<small>
														...
														<span id="view-count-{{ $val->id }}">{{ $count }}</span>
													</small>
												</div> 
											</div>
											<div class="mb-25">
