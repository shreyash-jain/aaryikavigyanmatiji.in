			...
			<div class="card">	
				<div class="container px-0">
					<div class="card-header ">
						<h2 class="card-title"> {{__('gallery.edit_image')}}</h2>						
					</div>
				</div>
				<div class="card-body">					
					<form  id="gallery_form" action="{{ route('meta_data.update',$data->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="row">
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select class="form-control" id="selectize-dropdown" name="page_name" >
											...
											<option value="{{$val->url}}" {{isset($data->page_name) && !empty($data->page_name) && $data->page_name == $val->url ?'selected':''}}>{{$val->name}}</option>
											@endforeach
										</select>
							...
							<div class="col-lg-6 col-sm-12">	
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="title" value="{{(isset($data->title) ? $data->title : '') }}" name="title" placeholder="{{__('meta_data.enter_title')}}">
										@if ($errors->has('title'))
							...
							<div class="col-lg-6 col-sm-12" >									
								<div  class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="keywords" value="{{(isset($data->keywords) ? $data->keywords : '') }}" name="keywords" placeholder="{{__('meta_data.enter_keywords')}}">
							...
							<div class="col-lg-6 col-sm-12" >									
								<div  class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="description" value="{{(isset($data->description) ? $data->description : '') }}" name="description" placeholder="{{__('meta_data.enter_description')}}">
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select class="form-control" id="selectize-dropdown" name="status" >
											...
											<option value="1" {{isset($data->status) && !empty($data->status) && $data->status == 1 ?'selected':''}}>{{__('meta_data.active')}}</option>
											<option value="2" {{isset($data->status) && !empty($data->status) && $data->status == 2 ?'selected':''}}>{{__('meta_data.unactive')}}</option>
										</select>
