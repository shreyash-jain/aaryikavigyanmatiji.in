			...
			<div class="card">	
				<div class="container px-0">
					<div class="card-header ">
						<h2 class="card-title"> {{__('oshadhi.edit_oshadhi')}}</h2>						
					</div>
				</div>
				<div class="card-body">					
					<form  id="gallery_form" action="{{ route('oshadhi_manjoosha.update',$data->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="row">
							<div class="col-lg-6 col-sm-12">	
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="name" value="{{(isset($data->name) ? $data->name : '') }}"  name="name" placeholder="{{__('oshadhi.enter_name')}}">
										@if ($errors->has('name'))
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select class="form-control" id="selectize-dropdown" name="status" >
											...
											<option value="1" {{isset($data->status) && !empty($data->status) && $data->status == 1 ?'selected':''}}>{{__('oshadhi.active')}}</option>
											<option value="2" {{isset($data->status) && !empty($data->status) && $data->status == 2 ?'selected':''}}>{{__('oshadhi.unactive')}}</option>
										</select>
							...
							<div class="col-lg-6 col-sm-12">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="order" value="{{(isset($data->order) ? $data->order : '') }}" name="order" placeholder="{{__('oshadhi.enter_order')}}">
										@if ($errors->has('order'))
							...
							<div class="col-lg-6 col-sm-12">	
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<textarea class="form-control" name="short_description" maxlength="525" rows="3"  placeholder="{{__('oshadhi.enter_short_description')}}" id="short_description">{{(isset($data->short_description) ? $data->short_description : '') }}</textarea>
							...
							<div class="form-group col-lg-12">
								...
								<div class="m-t-15">
									<textarea class="form-control" name="description" rows="3"  placeholder="{{__('oshadhi.enter_description')}}" id="summernote-standard">{{(isset($data->description) ? $data->description : '') }}</textarea>
								</div>	
