	...
	<div class="main-content">
		<div class="container-fluid">
			<div class="card">	
				...
				<div class="card-body">					
					<form  id="gallery_form" action="{{ route('gallery.update',$data->id) }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="row">
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select class="form-control" id="selectize-dropdown" name="status" >
											...
											<option value="1" {{isset($data->status) && !empty($data->status) && $data->status == 1 ?'selected':''}}>{{__('gallery.active')}}</option>
											<option value="2" {{isset($data->status) && !empty($data->status) && $data->status == 2 ?'selected':''}}>{{__('gallery.unactive')}}</option>
										</select>
							...
							<div class="col-lg-6 col-sm-12" >									
								<div  class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="image_url" value="{{(isset($data->image_url) ? $data->image_url : '') }}"  name="image_url" placeholder="{{__('gallery.enter_image_url')}}">
							...
							<div class="col-lg-6 col-sm-12">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="order" value="{{(isset($data->order) ? $data->order : '') }}" name="order" placeholder="{{__('gallery.enter_order')}}">
										@if ($errors->has('order'))
							...
							<div class="col-lg-6 col-sm-12">
								<div class="form-group row">								
									<label class="col-sm-3 col-form-label control-label text-sm-right">{{__('gallery.is_new')}}</label>
										<input class="ml-3" id="agreement" name="is_new" type="checkbox" value="1" {{ $data->is_new=="1" ? 'checked':'' }}>											
								</div>			
							</div>	
							<div class="col-lg-12">		
