			...
			<div class="card">
				<div class="container px-0">
					<div class="card-header ">
						<h2 class="card-title">{{__('classes.classes')}}  {{__('classes.classes_add')}}</h2>						
					</div>
				</div>
				<div class="card-body">					
					<form  id="classes_form" action="{{ route('classes.store') }}" method="POST" enctype="multipart/form-data">
					@csrf
						<div class="row">
							<div class="col-lg-6 col-sm-12">	
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="title" value="{{ old('title')}}" name="title" placeholder="{{__('classes.enter_title')}}">
										@if ($errors->has('title'))
							...
							<div class="col-lg-6 col-sm-12">										
								<div class="form-group row">		
									...
									<div class="col-sm-9">			
										<input type="text" class="form-control" id="url_slug" value="{{ old('url_slug')}}" name="url_slug" placeholder="{{__('classes.enter_url_slug')}}">		
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<select class="form-control" id="selectize-dropdown" name="status" >
											<option  disabled selected >-- {{__('classes.choose_status')}} --</option>			
											<option value="1" {{ old('status') == 1 ? 'selected' : '' }}>{{__('classes.active')}}</option>
											<option value="2" {{ old('status') == 2 ? 'selected' : '' }}>{{__('classes.unactive')}}</option>
										</select>
								...
								<div class="form-group row">	
									...
									<div class="col-sm-9">	
										<select class="form-control" id="selectize-dropdown" name="type" >		
											<option  disabled selected >-- {{__('classes.choose_type')}} --</option>				
											<option value="1" {{ old('type') == 1 ? 'selected' : '' }}>{{__('classes.video')}}</option>	
											<option value="2" {{ old('type') == 2 ? 'selected' : '' }}>{{__('classes.audio')}}</option>	
										</select>		
										@if ($errors->has('type'))		
							...
							<div class="col-lg-6 col-sm-12">
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<input type="text" class="form-control" id="order" value="{{ old('order')}}" name="order" placeholder="{{__('classes.enter_order')}}">
										@if ($errors->has('order'))
								...
								<div class="form-group row">
									...
									<div class="col-sm-9">
										<div class="icon-input">
											...
											<input  id="datepicker-1"  data-provide="datepicker" name="date"  type="text" class="form-control dateformate" placeholder="{{__('classes.enter_date')}}">
										</div>
										@if ($errors->has('date'))
							...
							<div class="col-lg-6 col-sm-12">	
								<div class="form-group row">	
									...
									<div class="col-sm-9"><input type="file" class="form-control" id="thumbnail" value="{{ old('thumbnail') }}" name="thumbnail" }}">			
										...
										<ul>
    										<li class="text-dark">{{__('classes.file_type_allowed')}} </li>
    										<li class="text-dark">{{__('classes.dimensions ')}} </li>
    										<li class="text-dark">{{__('classes.maximum_file_size')}} </li>
								    	</ul>
									</div>			
								</div>	
