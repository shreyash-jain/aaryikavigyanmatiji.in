                        ...
                        <div class="card">	
							<div class="container px-0">
								<div class="card-header ">
									<h3 class="card-title">{{__('sanghast_aaryika.sanghast_brahmacharya_sisters')}} {{__('sanghast_aaryika.list')}}</h3>
									<div class="card-toolbar">
										<div class="row text text-white-50 text-center">
											<div class=" align-items-center d-flex">									
												<div class=" text-end">
													<a href="{{ route('admin.dashboard') }}"  class="btn btn-gradient-danger">{{__('button.back')}}</a>	
													<a href="{{ route('sanghasth_brahmachary_bahane.create') }}"  class="btn btn-gradient-primary">{{__('sahitya.saahity_add')}}</a>
												</div>
											</div>	
										</div>
									</div>
										...
										@foreach($data as $key=>$val)
                                            <tr>                                                                                              
                                                ...
                                                <td class="text-center font-size-25">
                                                    <a href="{{ route('sanghasth_brahmachary_bahane.edit',$val->id) }}" class="text-gray m-r-15 btn-success btn-sm px-2 py-1"><i class="ti-pencil text-white"></i></a>
													<a data-bahan_id="{{$val->id}}" class="text-gray delete_bahan btn-danger btn-sm px-2 py-1"><i class="ti-trash text-white"></i></a>
                                                </td>
                                            </tr>
                                         @endforeach	    										@endif
                                        </tbody>
...
<script>
	$(document).on('click','.delete_bahan', function () {
		var BahanId = $(this).data("bahan_id");		
		var Action = "{{ route('sanghasth_brahmachary_bahane.destroy', '_bahan_id_') }}".replace('_bahan_id_', BahanId);
console.log(Action) ;	
	Swal.fire({
			title: are_you_sure,
